<x-layouts::app :title="__('Solutions')">
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a 
                href="{{ route('web.solutions.create') }}"
        ...
        @if ($solutions->isEmpty())
            <div class="bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-8 text-center">
                <p class="text-gray-500 dark:text-gray-400">
                    {{ __('No solutions found. Create one to get started.') }}
                </p>
            </div>
        @else
                        ...
                        @foreach ($solutions as $solution)
                            <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    <a href="{{ route('web.solutions.show', $solution) }}" class="hover:text-blue-600 dark:hover:text-blue-400">
                                        {{ $solution->name }}
                                    </a>
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    <a href="{{ route('web.solutions.show', $solution) }}" class="hover:text-blue-600 dark:hover:text-blue-400">
                                        {{ $solution->root_causes_count }} {{ __('Root Cause', ['count' => $solution->root_causes_count]) }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
