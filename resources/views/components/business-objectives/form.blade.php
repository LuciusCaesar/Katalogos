...
<form method="POST" action="{{ $businessObjective ? route('web.business-objectives.update', $businessObjective) : route('web.business-objectives.store') }}" class="space-y-6">
    @csrf
    ...
    <div>
        ...
        <input 
            ...
            id="name"
            name="name"
            value="{{ old('name', $businessObjective?->name) }}"
            required
            autofocus
    ...
    <div>
        ...
        >{{ old('description', $businessObjective?->description) }}</textarea>
        @error('description')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex justify-end">
        ...
        >
            {{ $businessObjective ? __('Update Business Objective') : __('Create Business Objective') }}
        </button>
    </div>
</form>
